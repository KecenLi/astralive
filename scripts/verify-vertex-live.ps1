...
try {
    ...
    if ($Uv) {
        ...
        Invoke-ApiCommand -Executable $Uv -Arguments @("run", "python", $TempScript) -Provider "vertex-ai" -CommandName "Gemini Live verification"
    } elseif ($Python) {
        Invoke-ApiCommand -Executable $Python -Arguments @($TempScript) -Provider "vertex-ai" -CommandName "Gemini Live verification"
    } else {
